...
#define F_CPU 16000000 // TODO work out the real value
...
void st_go_idle()
{
  ...
  if (((settings.stepper_idle_lock_time != 0xff) || sys_rt_exec_alarm || sys.state == STATE_SLEEP) && sys.state != STATE_HOMING) {
    ...
    // TODO the systick needs to run at a higher irq priority than tim2, else delay_ms turns into an infinite loop
...
void step_period_isr(void) 
{
  ...
  dirn_wr(st.dir_outbits);

  // Then pulse the stepping pins
  #ifdef STEP_PULSE_DELAY
    st.step_bits = st.step_outbits; // Store out_bits to prevent overwriting.
  #else  // Normal operation
    step_wr(st.step_outbits);
  #endif

  // Enable step pulse reset timer so that The Stepper Port Reset Interrupt can reset the signal after
  // exactly settings.pulse_microseconds microseconds, independent of the main Timer1 prescaler.
  // jth TCNT0 = st.step_pulse_time; // Reload Timer0 counter
  // jth TCCR0B = (1<<CS01); // Begin Timer0. Full speed, 1/8 prescaler
  ...
  if (st.exec_segment == NULL) {
    ...
    if (segment_buffer_head != segment_buffer_tail) {
      ...
      st.exec_segment = &segment_buffer[segment_buffer_tail];
        ...
        // jth TCCR1B = (TCCR1B & ~(0x07<<CS10)) | (st.exec_segment->prescaler<<CS10);
      #endif

      // Initialize step segment timing per step and load number of steps to execute.
      // jth OCR1A = st.exec_segment->cycles_per_tick;
...
void step_pulse_isr(void)
{
  ...
  {
    step_wr(st.step_bits); // Begin step pulse.
  }
#endif
